...
$free_space_before = (Get-WmiObject win32_logicaldisk -filter "DeviceID='C:'" | select-object Freespace).FreeSpace/1GB
...
$seconds_elapsed = 0
do {
    "waiting for cleanmgr to complete... ({0} seconds elapsed)" -f $seconds_elapsed
    start-sleep 10
    $seconds_elapsed += 10
} while ((get-wmiobject win32_process | where-object {$_.processname -eq 'cleanmgr.exe'} | measure-object).count)
 
$free_space_after = (Get-WmiObject win32_logicaldisk -filter "DeviceID='C:'" | select-object Freespace).FreeSpace/1GB
 
"Free Space Before Cleaning: {0:0.##} GB" -f $free_space_before
